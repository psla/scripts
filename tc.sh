...
# throughput / traffic control - politeness rules that always prioritize some protocols over others (on upload).
# note that this script does not control download at all
...
# here comes phone. No matter what, it's always first (as long as we don't fill up modem's buffer)
tc class add dev ${INET} parent 1:1 classid 1:10 htb rate ${PHONE_RATE} ceil 300kbit prio 0

# here come packets marked as high prio network traffic (skype? starcraft, quake, outcoming ssh, packages with SYN)
